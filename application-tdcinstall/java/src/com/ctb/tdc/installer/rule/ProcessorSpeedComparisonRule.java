 ...
 *
 */
public class ProcessorSpeedComparisonRule extends CustomCodeRule {
    ...
    public boolean evaluateRule() {
        ...
        if( (strOperand1 == null) && (strOperand2 == null) && (strOperator == null) ) {
            System.err.println("ProcessorSpeed Comparison Rule: ignored!");
            ...
            System.err.println("ProcessorSpeed Comparison Rule: " + strOperand1 + " " + strOperator + " " + strOperand2);
            try {
                float operand1 = 0;
                int index = strOperand1.indexOf("MHz");
                if (index > 0) {
                    strOperand1 = strOperand1.replaceAll("MHz", "").trim();
                    operand1 = Float.parseFloat(strOperand1) * 1000000;
                }
                else {
                    index = strOperand1.indexOf("GHz");
                    if (index > 0) {
                        strOperand1 = strOperand1.replaceAll("GHz", "").trim();
                        operand1 = Float.parseFloat(strOperand1) * 1000000000;                
                    }
                }
                
                float operand2 = Float.parseFloat(strOperand2);
                System.err.println("ProcessorSpeed Comparison Rule: " + operand1 + " " + strOperator + " " + operand2);
